use dabgent_screenshot::ScreenshotOptions;
...
async fn test_screenshot_smoke() {
    ...
    use dabgent_screenshot::screenshot_app;
    ...
    let template_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap()
        .join("dabgent_templates")
        .join("template_trpc");

    if !template_dir.exists() {
        panic!(
            "Template not found at {}. This test expects the dabgent_templates crate.",

use anyhow::Result;
use integrations::DatabricksRestClient;
...
async fn main() -> Result<()> {
        ...
        Ok(results) => {
            for row in results {
                println!("Total Revenue: ${}", 
                    DatabricksRestClient::format_value(row.get("total_revenue").unwrap_or(&serde_json::Value::Null)));
                println!("Total Orders: {}", 
                    DatabricksRestClient::format_value(row.get("total_orders").unwrap_or(&serde_json::Value::Null)));
                println!("Unique Customers: {}", 
                    DatabricksRestClient::format_value(row.get("unique_customers").unwrap_or(&serde_json::Value::Null)));
                println!("Average Order Value: ${}", 
                    DatabricksRestClient::format_value(row.get("average_order_value").unwrap_or(&serde_json::Value::Null)));
            }
        }
    ...
    match client.get_table_details(table_name, 3).await {
        Ok(details) => {
            ...
            println!("Row Count: {}", details.row_count.map(|c| c.to_string()).as_deref().unwrap_or("N/A"));

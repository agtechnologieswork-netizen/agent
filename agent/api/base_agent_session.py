...
class BaseAgentSession(AgentInterface, ABC):
    ...
    def _extract_llm_messages(self, thread: List[InternalMessage]) -> List[InternalMessage]:
        """Extract LLM text messages from thread."""
        messages = []
        if thread:
            for msg in thread:
                    ...
                    for content_block in msg.content:
                        if hasattr(content_block, 'text'):
                            messages.append(InternalMessage(
                                role="assistant",
                                content=[TextRaw(content_block.text)]
                            ))
        return messages
                    ...
                    case FSMStatus.COMPLETED:
                        try:
                            ...
                            if not is_diff_meaningful:
                                ...
                                # Include LLM's explanatory messages if available
                                messages_to_send = self._extract_llm_messages(thread)

                                if not messages_to_send:
                                    messages_to_send = [InternalMessage(
                                        role="assistant",
                                ...
                                # Send LLM messages first if available
                                if messages_to_send:
                                    await self.send_event(
                                        event_tx=event_tx,
                                        status=AgentStatus.RUNNING,

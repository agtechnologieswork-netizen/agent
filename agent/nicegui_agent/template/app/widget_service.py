...
class WidgetService:
    ...
    @staticmethod
    def create_widget_from_template(template_id: int, page: str = "dashboard", name: Optional[str] = None) -> Optional[Widget]:
    ...
    def load_user_preset(preset_id: int, page: str = "dashboard") -> bool:
        ...
        with Session(engine) as session:
            ...
            existing_widgets = session.exec(
                select(Widget).where(Widget.page == page)
            ).all()
    ...
    @staticmethod
    def initialize_default_widgets():
        """Initialize default widgets if none exist"""
        with Session(engine) as session:
            # Check if widgets already exist
            widget_count = session.exec(select(Widget)).first()
            if widget_count:
                return
            ...
            default_widgets = [
                {
                    ...
                    "config": {
                        "content": "# Welcome to Your Dashboard\nCustomize this page by adding, removing, or rearranging widgets.",
                        "markdown": True
                    }
                },
                {
            ...
            for i, widget_data in enumerate(default_widgets):
                widget = Widget(
                    name=widget_data["name"],
                    type=widget_data["type"],
            ...
            session.commit()
            logger.info("Initialized default widgets")
...
class LaravelActor(BaseActor, LLMActor):
    ...
    def tools(self) -> list[Tool]:
        return [
            ...
            {
                "name": "npm_install",
                "description": "Install Node.js packages via npm",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "packages": {"type": "array", "items": {"type": "string"}},
            ...
            {
                "name": "artisan_make",
                "description": "Generate Laravel files using artisan",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "type": {"type": "string", "enum": ["model", "controller", "migration", "seeder", "factory"]},
                        "name": {"type": "string"},
                    },
                    "required": ["type", "name"],
                },
            ...
            {
                "name": "complete",
                "description": "Mark the task as complete. This will run tests and checks to ensure the changes are correct.",
                "input_schema": {
                    "type": "object",
                    "properties": {},
                },
        ...
        for block in node.data.head().content:
            ...
            try:
                ...
                match block.name:
                    ...
                    case "delete_file":
                        node.data.workspace.rm(block.input["path"])
                        node.data.files.update({block.input["path"]: None})
                        result.append(ToolUseResult.from_tool_use(block, "success"))
                    case "composer_require":
                        packages = block.input["packages"]
                        ...
                        if exec_res.exit_code != 0:
                            result.append(
                                ToolUseResult.from_tool_use(
                                    block,
                                    f"Failed to install packages: {exec_res.stderr}",
                                    is_error=True,
                                )
                            )
                        else:
                            node.data.files.update(
                                {
                                    "composer.json": await node.data.workspace.read_file(
                                        "composer.json"
                                    )
                                }
                            )
                    ...
                    case "npm_install":
                        ...
                        if exec_res.exit_code != 0:
                            ...
                            node.data.files.update(
                                {
                                    "package.json": await node.data.workspace.read_file(
                                        "package.json"
                                    )
                                }
                            )
                            result.append(ToolUseResult.from_tool_use(block, "success"))
                    case "artisan_make":
                        artisan_type = block.input["type"]
                        name = block.input["name"]
                        exec_res = await node.data.workspace.exec_mut(
                            ["php", "artisan", f"make:{artisan_type}", name]
                        )
                        if exec_res.exit_code != 0:
                            result.append(
                                ToolUseResult.from_tool_use(
                                    block,
                                    f"Failed to generate {artisan_type}: {exec_res.stderr}",
                                    is_error=True,
                                )
                            )
                        else:
    ...
    async def run_checks(self, node: Node[BaseData], user_prompt: str) -> str | None:
        phpunit_result = await node.data.workspace.exec(["./vendor/bin/phpunit"])
        if phpunit_result.exit_code != 0:
            return f"PHP tests failed:\n{phpunit_result.stdout}\n{phpunit_result.stderr}"
        
        npm_test_result = await node.data.workspace.exec(["npm", "run", "types"])
        if npm_test_result.exit_code != 0:
            return f"TypeScript checks failed:\n{npm_test_result.stdout}\n{npm_test_result.stderr}"
        return None

    @property
    def files_allowed(self) -> list[str]:
        return ["app/", "resources/", "routes/", "database/", "tests/", "config/"]
    ...
    async def get_repo_files(
        ...
        repo_files.update(f"resources/{file_path}" for file_path in await workspace.ls("resources"))
        repo_files.update(f"routes/{file_path}" for file_path in await workspace.ls("routes"))
        repo_files.update(f"database/{file_path}" for file_path in await workspace.ls("database"))

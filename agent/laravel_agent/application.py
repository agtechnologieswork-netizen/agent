...
class ApplicationContext(Context):
    """Context for the Laravel application state machine"""
...
class FSMApplication:
    ...
    def base_execution_plan(cls) -> str:
        return "\n".join(
            [
                "1. Laravel application development based on user requirements.",
                "",
                "The result application will be based on PHP Laravel framework with Inertia.js and React frontend. The application can include various backend APIs, frontend components, database models, and modern tooling.",
            ]
        )
    ...
    async def make_states(
        ...
        workspace = await Workspace.create(
            ...
            context=client.host().directory("./laravel_agent/template"),
            setup_cmd=[
                ["apt-get", "update"],
    ...
    async def get_diff_with(self, snapshot: dict[str, str]) -> str:
        ...
        template_dir = self.client.host().directory("./laravel_agent/template")
...
async def main(user_prompt="Simple Laravel blog application"):
    async with dagger.Connection(
        ...
        if fsm_app.maybe_error():
            ...
            await fsm_app.apply_changes("Add user authentication")
